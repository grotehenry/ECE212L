...
module LEDMatrixTopLevel(
    input logic clk,rst_raw, np_raw,
    input logic [2:0] NEXTCOLOR,
    output logic sclk,lat,blank,R1,B1,G1,R2,B2,G2,C,B,A,
    ...
    pixel_gen pg(.NEXTPB(NEXTPB),.NEXTCOLOR(NEXTCOLOR),.clk(clk),.rst(rst),.col(col), .row(row), .R1(R1), .G1(G1), .B1(B1), .R2(R2), .G2(G2), .B2(B2),.num(num));
